...
function pack (tarball, folder, pkg, cb) {
  ...
  readJson(path.join(folder, 'package.json'), function (er, pkg) {
    if (er || !pkg.bundleDependencies) {
      ...
      readPackageTree(folder, pulseTillDone('pack:readTree:' + pkg.name,iferr(cb, function (tree) {
        var recalcGroup = log.newGroup('pack:recalc:' + pkg.name)
        recalculateMetadata(tree, recalcGroup, iferr(cb, function () {
          recalcGroup.finish()
          pack_(tarball, folder, tree, flattenTree(tree), pkg, pulseTillDone('pack:' + pkg.name, cb))
        }))
      })))
    }
  })
